...
fun marketOrderBooksDataModelToDomainModel(marketOrderBooksDataModel: MarketOrdersDataModel): MarketOrdersDomainModel {
    val sellSum = (
        (
            marketOrderBooksDataModel.sell?.sumOf {
                (it.volume ?: 0) * (it.count ?: 0)
            } ?: 1
        ).toFloat() / (marketOrderBooksDataModel.sell?.size?.toFloat() ?: 1f)
    )

    val buySum = (
        (
            marketOrderBooksDataModel.buy?.sumOf {
                (it.volume ?: 0) * (it.count ?: 0)
            } ?: 1
        ).toFloat() / (marketOrderBooksDataModel.buy?.size?.toFloat() ?: 1f)
    )

    return MarketOrdersDomainModel(
        ask =
            marketOrderBooksDataModel.sell?.map {
        ...
        bid =
            marketOrderBooksDataModel.buy?.map {
                MarketOrderDomainModel(
                    price = (it.rate_f ?: "").decimalNumberSeparator(),
